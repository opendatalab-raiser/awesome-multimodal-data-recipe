...
使用生成模型（如Stable Diffusion）创建新图像，与文本配对构建训练数据。

**⚠️ 重要区分**:
- ✅ **收录**: 使用T2I模型生成图像用于VLM训练的论文
- ❌ **不收录**: 纯生成模型论文（如DALL-E、Stable Diffusion本身）
- ❌ **不收录**: 数据收集/获取（非合成）

#### 🎨 基于扩散模型的训练数据生成

> 使用扩散模型（如Stable Diffusion）生成训练图像

- **📄 StableRep** [(arxiv 2306.00984)](https://arxiv.org/abs/2306.00984)
  - **数据合成方法**（Section 3）:
    - 使用**Stable Diffusion**生成20M合成图像
    - 基于ImageNet类别名称作为prompts
    - 研究合成数据训练视觉表征的有效性
  - **关键发现**: 纯合成数据也能训练强大的表征学习模型
  - **开源**: ✅ [Code](https://github.com/google-research/syn-rep-learn)

- **📄 Synthetic Data from Diffusion Models Improves ImageNet Classification** [(arxiv 2304.08466)](https://arxiv.org/abs/2304.08466)
  - **数据合成方法**（Section 3）:
    - 使用Guided Diffusion生成ImageNet类别图像
    - 专注于补充长尾类别数据
    - 真实+合成混合训练
  - **关键发现**: 合成数据显著改善稀缺类别
  - **论文**: [arxiv](https://arxiv.org/abs/2304.08466)
