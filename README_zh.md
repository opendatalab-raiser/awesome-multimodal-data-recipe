...
- **📄 TextFlux** [(arXiv 2505.17778)](https://arxiv.org/abs/2505.17778) 🏷️ **[方法]**
...
- **📄 DocLayout-YOLO** [(arXiv 2410.12628)](https://arxiv.org/abs/2410.12628) 🏷️ **[方法 + 数据]**
...
- **📄 Multimodal Self-Instruct** [(arXiv 2407.07053)](https://arxiv.org/abs/2407.07053) 🏷️ **[方法 + 数据]**
...
- **📄 GraspVLA** [(arXiv 2505.03233)](https://arxiv.org/abs/2505.03233) 🏷️ **[方法 + 数据]**
...
- **📄 GenLLaVA** [(arXiv 2406.11262)](https://arxiv.org/abs/2406.11262) 🏷️ **[方法 + 数据]**
...
- **📄 COMO** 🏷️ **[方法 + 数据]**
  - **聚焦**: **通过多模态反事实样本增强视觉-语言概念理解** - 同时生成文本和视觉反事实样本，改善VL模型超越"bag-of-objects"局限的概念理解能力
  - **数据合成方法** - **多模态反事实生成Pipeline**:
    - **核心创新**: 首个同时生成文本和视觉反事实样本的框架，解决VL模型在属性、关系、词序理解方面的不足
    - **反事实文本生成**:
      - 使用**BERT**掩码语言模型替换原始标注中的关键概念词（名词、动词、形容词）
      - **NLP解析**: 使用spaCy识别对象（名词）、关系（动词/副词）、属性（形容词）
      - **词汇替换策略**: 掩码选定概念 → 检索最可能替换 → 使用WordNet过滤同义词
      - **质量控制**: 选择最高分候选词，创建真正负样本（非同义替换）
    - **反事实图像生成**:
      - 使用**Stable Diffusion**根据反事实标注生成对应反事实图像
      - **图像选择**: 生成10个候选，基于双重CLIP相似度选择最佳: score = ε_T(T_c)·ε_V(V_i) + ε_V(V_o)·ε_V(V_i)
      - **质量保证**: 确保改变概念（因果）可学习，同时保留未改变概念（伪相关）
    - **对比学习增强**:
      - **反事实引导损失**: 将事实和反事实对放入同一mini-batch作为困难负样本
      - **加权困难负样本损失**: 使用归一化相似度动态重新加权: α_ij = (n-1)·S(T_i,V_j) / Σ_k≠i S(T_i,V_k)
      - **双重对比目标**: 将反事实嵌入推离事实锚点，同时匹配反事实图像-文本对
  - **数据规模**: 
    - **基础数据集**: MS-COCO (11.3万图像，56.7万标注)
    - **COCO-CF数据集**: 68万图像 + 113.4万标注（增加56.7万反事实图像-文本对）
    - **增强比例**: 标注多样性增加5倍，图像多样性增加6倍
  - **实验结果**:
    - **VL-Checklist**: 属性+3.17%，关系+2.12%，对象+1.70%，超越最佳基线
    - **Winoground**: 文本+1.60%，图像+4.35%，组合+2.63%提升
    - **组合推理**: 在理解空间关系、对象属性、词序变化方面显著提升
    - **消融发现**: 文本和图像反事实都必要；In-Batch策略优于随机采样
  - **机构**: 西安电子科技大学计算机科学与技术学院
  - **开源**: ✅ [数据集与代码](https://github.com/laichengen/COMO)
...
- **📄 增强视觉指令微调** [(ACL Findings 2024)](https://aclanthology.org/2024.findings-acl.864/) 🏷️ **[方法 + 数据]** - **ACL Findings 2024**
  - **聚焦**: **合成图像-对话数据增强视觉指令微调** - 使用ChatGPT和Stable Diffusion生成全面的图像-对话数据集，提升多模态LLM在多种视觉任务上的能力
  - **数据合成方法** - **双阶段ChatGPT引导生成Pipeline**:
    - **核心创新**: 首个系统性利用LLM (ChatGPT)同时生成Stable Diffusion提示词和对应对话数据的方法，解决现有视觉指令数据集在能力覆盖和对话多样性方面的局限
    - **阶段1: ChatGPT提示词与对话生成**:
      - **提示词生成**: 使用**ChatGPT**配合能力特定指令生成针对11种视觉能力的Stable Diffusion提示词
      - **能力类别**: 动物、动作、颜色、异常检测、场景、风格、食物、职业、车辆、家具、植物识别
      - **提示工程技术**:
        - **长度控制**: 每个提示词最多10个关键词，确保清晰聚焦和准确图像生成
        - **关键词优先级**: 最关键词汇放在开头，双括号标记以强调
        - **稳定性增强**: 添加能力特定指令和注意事项来鼓励多样性
        - **异常图像引导**: 对于笑话理解任务，引导ChatGPT创建"异常图像"提示（如"长颈鹿走过狭窄走廊"）
      - **对话生成**: 生成对应的问答对，包含约束条件:
        - **长度限制**: 每个对话≤500字符以保持简洁  
        - **多样性要求**: 包含误导性二选一问题以增加复杂度
        - **任务特定限制**: 解决模型偏差（如建筑工人→添加人体属性避免聚焦建筑物）
    - **阶段2: Stable Diffusion图像生成**:
      - **模型**: **Stable Diffusion v1.4**用于文本到图像生成
      - **生成设置**: 每个提示词生成多个候选，基于对齐分数选择最佳
      - **多图像数据**: 同时生成配对提示词用于图像相似性/差异性/逻辑关系任务（3K对）
      - **质量控制**: 基于CLIP的对齐验证，阈值γ=0.25
    - **高级数据类型**:
      - **交错多轮对话**: 带图像输入的逐步指导性对话，每轮都有视觉反馈
      - **多图像任务**: 相似性比较、差异分析、逻辑关系推理
      - **专门内容**: 异常图像检测、职业识别、细粒度属性理解
  - **技术架构集成**:
    - **基础模型**: LLaVA架构，包含CLIP视觉编码器 + 投影层 + LLM (Vicuna)
    - **训练策略**: 两阶段微调（模态对齐 + 视觉指令微调）
    - **令牌处理**: 通过线性/MLP投影将视觉特征转换为语言嵌入令牌
  - **数据规模**:
    - **主数据集**: **38K图像-对话对**，涵盖11个能力类别
    - **多图像数据**: **3K专门的多图像实例**
    - **交错对话**: 带视觉反馈的逐步指导性对话
    - **总体覆盖**: 与原始LLaVA数据集结合的综合训练集，增强能力
  - **评估与结果**:
    - **基准测试**: VizWiz (58.4% vs 53.6% LLaVA-1.5), MM-Vet (36.1% vs 35.4%), MME (1532.3 vs 1531.3), MMBench (69.4% vs 67.7%)
    - **人工评估**: 所有11种能力均有显著提升，特别是异常检测（+25%）、风格识别（+50%）
    - **GPT-4评分**: 持续性能提升，包含详细的能力特定分析
    - **多图像任务**: 差异检测（2.7→3.6）、相似性分析（2.2→2.8）、逻辑关系（3.1→3.7）显著提升
  - **关键技术贡献**:
    - **自动化Pipeline**: 首个结合LLM提示词生成与扩散模型图像合成的全自动方法
    - **能力特定设计**: 针对代表性不足的视觉任务（笑话、异常检测、多图像推理）进行定向生成
    - **质量保证**: 多层过滤，包括CLIP对齐、长度约束、任务特定限制
    - **可扩展性**: 基于模板的方法，易于扩展到新能力和领域
  - **机构**: 悉尼科技大学、腾讯、南洋理工大学、浙江大学、北京交通大学、西湖大学、鹏城实验室
  - **开源**: ✅ 数据集和生成模板（见补充材料）

---

- **📄 Ultimate3D** [(arXiv 2507.08513)](https://arxiv.org/abs/2507.08513) 🏷️ **[方法 + 数据]**
...
- **📄 2.5 Years in Class** [(arXiv 2501.00958)](https://arxiv.org/abs/2501.00958) 🏷️ **[方法 + 数据]** - **浙江大学 + 阿里达摩院**
...
- **📄 MM-IFEngine** [(ICCV 2025)](https://arxiv.org/abs/2411.17194) 🏷️ **[方法 + 数据]** - **ICCV 2025**
  - **聚焦**: **多模态指令遵循数据生成引擎** - 自动生成带复杂约束的高质量多模态指令遵循数据和评估基准
  - **数据合成方法** - **LLM驱动的约束集成与验证Pipeline**:
    - **核心创新**: 首个系统化的多模态指令遵循(MMIF)数据生成引擎，通过LLM自动整合多样化约束到视觉指令中，并构建综合评估基准
    - **问题识别**:
      - 现有MMIF训练数据稀缺且约束简单（如VisIT-Bench、MIA-Bench仅包含原子级约束）
      - 约束与视觉内容关联弱，导致模型在真实应用场景中指令遵循能力不足
      - 评估方法不精确：LLM-as-a-judge在需要精确输出约束（如字数）的任务上存在误判
    - **四步生成Pipeline**:
      1. **图像过滤（Step 1）**:
         - **目标**: 选择语义丰富的自然场景图像
         - **数据源**: CC3M、ALLaVA、MultiUI、Geo170k、ChartQA等多源数据
         - **质量指标**:
           - **IC9600指标**: 评估图像语义复杂度（信息量、类别多样性等）
           - **RAM指标**: 识别图像中的显著对象和场景元素
         - **筛选标准**: 优先选择具有丰富语义内容的自然场景图像（非低分辨率、非简单背景）
         - **理由**: 丰富的语义内容支持设计更全面、复杂的指令遵循任务
      2. **任务生成（Step 2）**:
         - **对于有原始QA的数据集（如ALLaVA）**:
           - 使用正则表达式和长度限制过滤问题
           - 排除包含few-shot示例的问题（Q_fs）
           - 排除包含选项的问题（Q_op）
           - 公式：Q_s = {q∈Q | q∉Q_fs ∧ q∉Q_op}
           - 获得适合约束集成的任务指令集T*
         - **对于无原始QA的数据集（如CC3M）**:
           - 设计预定义的任务指令模板池P_T
           - 包含描述、创作、分析等多类任务
      ...
      3. **约束集成（Step 3 - 核心步骤）**:
         - **约束池P_C构建**:
           - **6大类32子类约束**，形成多层级约束分类体系：
             a) **格式约束**: JSON格式、列表格式、代码块格式、段落结构
             b) **长度约束**: 字数限制、句子数限制、段落数限制
             c) **风格约束**: 正式/非正式语气、幽默风格、诗歌体、学术风格
             d) **关键词约束**: 必须包含特定词、避免特定词、使用同义词
             e) **组合约束**: 结合多种约束类型的复杂要求
             f) **感知约束**: 基于视觉内容的约束（颜色、对象、位置等）
           - **约束与视觉内容关联**: 确保约束与图像语义相关，避免冲突
         - **约束采样与生成**:
           - 从约束池P_C中采样k个约束类型（通常3-12个）
           - **两阶段选择策略**（针对有原始QA的数据）:
             - 第一阶段：从P_C中粗选大部分兼容的约束类型
             - 第二阶段：LLM精选并生成具体约束内容
           - **直接采样策略**（针对无原始QA的数据）:
             - 直接采样k个约束类型
             - LLM生成兼容的具体约束内容
         - **约束整合**:
           - LLM（GPT-4o）将选定的约束自然地融入任务指令
           - 确保约束之间相互兼容（无矛盾）
           - 输出包含多个约束的复杂指令
      4. **答案生成与质量控制（Step 4）**:
         - **答案生成**: GPT-4o基于图像和约束指令生成高质量答案
         - **质量控制**:
           - **准确率阈值**: 80%（通过GPT-4o自评估）
           - **约束验证**: 生成的答案必须满足所有指定约束
           - **多轮迭代**: 不满足质量要求的样本被丢弃或重新生成
    - **关键技术优势**:
      - **复杂约束**: 平均每个样本包含6.5个约束，远超现有数据集（通常1-2个）
      - **视觉-约束关联**: 约束设计考虑图像内容，确保指令的合理性和可执行性
      - **自动化**: 端到端自动化生成，无需人工标注
      - **可扩展性**: 约束池可灵活扩展，支持新增约束类型
  - **MM-IFInstruct-23K数据集**:
    - **总规模**: 23K高质量指令遵循数据
    - **数据来源分布**:
      - CC3M: 16K（70%）
      - ALLaVA: 6K（26%）
    ...
    - **约束统计**:
      - **平均约束数**: 6.5个/样本
      - **约束数分布**: 3-12个约束/样本，形成系统化的复杂度梯度
      - **约束类型分布**: 6大类32子类，覆盖多维度指令遵循要求
    - **数据特点**:
      - 包含真实应用场景常见的复杂约束（如"输出为JSON格式"、"使用至少2个同义词"等）
      - 约束组合多样化，训练模型同时满足多个约束的能力
  - **MM-IFDPO-23K偏好数据集**:
    - **目标**: 用于Direct Preference Optimization (DPO)训练
    - **构建方法**:
      - **Chosen样本**: 直接使用MM-IFInstruct-23K的高质量数据
      - **Rejected样本**: 4种负样本生成策略
        1. 随机删除33%的约束（保留67%）
        2. 随机删除66%的约束（保留34%）
        3. 删除所有约束（保留0%）
        4. 不提供图像输入，仅基于文本生成回答
      - 使用Qwen2-VL-7B-Instruct生成rejected回答
    - **规模**: 23K (chosen, rejected)对
    - **效果**: DPO训练进一步提升指令遵循精确度
  - **MM-IFEval评估基准**:
    - **规模**: 400个高质量评估样本
    - **约束分类**: 6大类32子类，形成完整的指令遵循评估体系
    - **问题类型**:
      - **Compose-Level**: 文本约束为主（300样本）
      - **Perception-Level**: 需要较强视觉感知能力的约束（100样本）
        - 包含自然场景、用户界面、图表、数学表达式等多种图像类型
    - **混合评估方法**（创新点）:
      1. **基于规则的评估**: 用于精确约束（如字数、格式）
         - 设计预定义函数验证输出是否满足约束
         - LLM提取相关参数，函数执行验证
      2. **LLM直接判断**: 用于可明确验证的约束（如关键词包含）
      3. **对比判断**: 用于主观约束（如语气、风格）
         - 生成无约束的对照输出
         - LLM比较两个输出，判断哪个更符合约束
      - **优势**: 避免LLM-as-a-judge在精确约束上的误判，提高评估准确性
  - **实验结果** - **开源模型微调后显著提升，保持VQA能力**:
    - **在MM-IFEval上的表现**:
      - Qwen2-VL-7B基线: 42.0% (Compose) / 80.5% (Overall)
      - +MM-IFInstruct-23K: 52.3% (+10.3%)
      - +MM-IFDPO-23K: **52.2%** (+10.2%)
      - LLaVA-NeXT-Llama3-8B基线: 39.7%
      - +MM-IFDPO-23K: **49.3%** (+9.6%)
    - **跨基准泛化（MIA-Bench）**:
      - Qwen2-VL-7B: 基线80.5% → 微调**88.1%** (+7.6%)
      - LLaVA-NeXT-Llama3-8B: 基线83.3% → 微调**90.0%** (+6.7%)
    - **文本指令遵循（IFEval）**:
      - Qwen2-VL-7B: 基线47.4% → 微调**59.7%** (+12.3%)
      - LLaVA-NeXT-Llama3-8B: 基线50.7% → 微调**69.1%** (+18.4%)
    - **VQA能力保持**（关键验证）:
      - 在MMMU、MMBench、MMStar、MMT-Bench、AI2D、OCRBench、MMVet、POPE等8个VQA基准上
      - 微调后模型性能与基线相当或略有提升
      - **结论**: MM-IFDPO-23K训练不会损害模型原有的视觉理解能力
    - **商业模型对比**（MM-IFEval）:
      - GPT-4o: 64.6% (Compose) / 44.0% (Perception)
      - Claude-3.5-Sonnet: 67.5% (Compose) / 44.0% (Perception)
      - 微调的Qwen2-VL-7B: 55.2% (Compose) / 43.0% (Perception)
      - **结论**: 开源7B模型微调后在Compose-Level上接近商业模型，但Perception-Level仍有差距
  - **消融实验关键发现**:
    - **DPO负样本策略对比**:
      - 删除100%约束（无约束）的负样本效果最佳
      - 删除33%或66%约束的负样本也有效
      - 无图像输入的负样本效果略差
      - **结论**: 明确的正负对比（有约束vs无约束）对DPO训练最有效
    - **数据规模影响**: 23K数据量达到较好的性能-成本平衡点
    - **约束数量影响**: 3-12个约束的梯度分布有助于模型学习不同复杂度的指令遵循
  - **机构**: 复旦大学、上海人工智能实验室、上海交通大学、香港中文大学
  - **作者**: Shengyuan Ding, Shenxi Wu, Xiangyu Zhao, Yuhang Zang, Haodong Duan, Xiaoyi Dong, Pan Zhang, Yuhang Cao, Dahua Lin, Jiaqi Wang
  - **发布时间**: ICCV 2025
  - **开源**: ✅ [代码和数据](https://github.com/xxx/MM-IFEngine) （GitHub链接待确认）
  - **意义**:
    - **问题解决**: 系统化解决多模态指令遵循训练数据稀缺和评估方法不精确的问题
    - **方法创新**: 
      - 提出首个约束池驱动的多模态指令数据自动生成框架
      - 创新的混合评估方法（规则+LLM+对比），解决LLM-as-a-judge的局限性
    - **数据贡献**: 
      - 提供23K高质量复杂约束指令数据（MM-IFInstruct-23K）
      - 提供23K DPO偏好数据（MM-IFDPO-23K）
      - 提供400样本综合评估基准（MM-IFEval）
    - **实用价值**: 
      - 显著提升开源模型的指令遵循能力，缩小与商业模型差距
      - 不损害原有VQA能力，可直接应用于实际应用
      - 约束池可扩展，支持定制化指令遵循能力培养

---

- **📄 CIRHS** [(arXiv 2507.05970)](https://arxiv.org/abs/2507.05970) 🏷️ **[方法 + 数据]**
...
- **📄 SMMQG** [(EMNLP 2024 Findings)](https://aclanthology.org/2024.findings-emnlp.759/) 🏷️ **[方法 + 数据]**
  - **聚焦**: **合成多模态问答生成** - 为多模态检索增强生成(MMRAG)系统生成风格和模态可控的评估数据集
  - **数据合成方法** - **Retriever + LLM + LMM协同生成Pipeline**:
    - **核心创新**: 首个可控制问题风格和模态的合成多模态QA数据生成框架，无需人工标注即可生成高质量评估数据
    - **五步生成Pipeline**:
      1. **种子源采样**:
         - 从多模态源S（文本段落、表格、图像）中采样种子源s_seed
         - 使用加权采样避免离群值：w_i基于源与其k近邻的语义相似度（E5-Large嵌入）
         - 确保生成的问题主题连贯性
      2. **实体提取**:
         - 使用GPT-4-Turbo从种子源中提取关键实体（如"tennis"、"Japan"、"machine learning"）
         - 对图像源使用图像描述和图像标题
         - 高温度(1.0)提升实体多样性，增强问题多样性
      3. **候选源检索**:
         - 使用提取的实体作为查询，检索候选源集合Z̃
         - 根据模态要求M（如M=[1,2,0]表示1个文本+2个表格）检索相应数量的各模态源
         - 文本/表格使用E5-Large，图像使用图像描述进行文本检索
      4. **问题生成**:
         - **输入**: 候选源Z̃、问题风格描述v、模态要求M、3个风格特定的few-shot示例
         - **模型**: GPT-4-Turbo（文本/表格）或GPT-4-Turbo with Vision（包含图像）
         - **任务**: 从Z̃中选择问题源Z⊆Z̃，生成问题q和答案a，输出源引用
         - **Prompt设计**: 明确要求问题必须基于选定源，遵循风格描述，不可移除任何选定源后仍可回答
      5. **验证步骤**:
         - 验证问题是否符合指定风格
         - 验证答案是否可从问题源推断
         - 使用GPT-4-Turbo进行二元Pass/Fail判断
    - **五种问题风格**:
      1. **信息提取(Information Extraction)**: 从单一源提取并返回信息
      2. **比较对比(Compare Contrast)**: 比较两个密切相关实体或主题
      3. **数值计算(Numerical)**: 基于源中数字进行计算
      4. **复合问题(Compound)**: 两个松散相关的信息提取问题，用"and"连接
      5. **多跳推理(Multi-hop)**: 需先解决隐含子问题，再解决完整问题
    - **多跳问题特殊生成**:
      - 先生成两个中间问题：问题1关于实体，问题2答案是实体
      - 使用LLM组合中间问题和答案，形成多跳问题
      - 跨模态多跳：按模态分割候选源；单模态多跳：随机分割
    - **模态支持**: 文本、表格、图像及其所有成对组合（9种模态组合）
    - **图像处理策略**:
      - **图像描述化**: 使用LLaVA-13B生成图像描述，使基于文本的模型能够检索和推理图像
      - **三元组表示**: 图像源 = (图像本身, 图像标题, 图像描述)
    - **关键技术优势**:
      - **风格控制**: 通过风格描述v和few-shot示例精确控制问题类型
      - **模态控制**: 通过模态要求M精确控制问题涉及的源类型组合
      - **无人工标注**: 完全自动化生成，无需crowdsourcing
      - **可扩展性**: 可应用于任何多模态文档集合
      - **领域定制**: 通过控制源文档实现领域特定数据生成
  - **数据规模**:
    - **Wikipedia QA数据集**: 1024个QA对，5种风格，9种模态组合
    - **Biology数据集**: 额外生成的大学生物教材数据集（展示领域定制能力）
  - **人类研究结果** - **质量与众包基准MMQA相当**:
    - **问题流畅度(1-5分)**: SMMQG **4.53** vs MMQA 3.68 (+0.85, p<0.001*)
    - **风格忠实度(%)**: SMMQG **98.3%** vs MMQA 96.7% (+1.6%)
    - **源相关性(%)**: SMMQG **93.0%** vs MMQA 85.8% (+7.2%)
    - **可回答性(%)**: SMMQG **94.7%** vs MMQA 85.8% (+8.9%, p=0.02*)
    - **答案正确性(%)**: SMMQG **92.7%** vs MMQA 80.0% (+12.7%, p=0.001*)
  - **模型评估实验** - **揭示风格和模态特定的性能洞察**:
    - **Retriever评估**（Recall@5）:
      - **E5-Large**: 在所有模态上表现最佳（纯文本98.8%，文本-文本91.5%）
      - **OpenCLIP**: 在纯图像检索上最佳（84.0%）
      - **BM25**: 在非文本模态上表现较差
    - **QA模型评估**（GPT-4-Turbo-judge评分）:
      - **GPT-4-Turbo**: 全面领先（信息提取99.3%，多跳96.2%）
      - **Claude-3-Opus**: 第二佳（信息提取96.1%，多跳88.7%）
      - **开源模型**: Vicuna-13b+LLaVA-13b在开源模型中表现最佳
    - **风格特定洞察**:
      - 所有模型在**比较对比**和**数值计算**任务上表现较弱
      - **多跳推理**对大多数模型具有挑战性
      - **信息提取**相对简单，大多数模型表现良好
    - **模态特定洞察**:
      - 纯文本任务表现最佳
      - **表格QA**对所有模型具有挑战性
      - Gemini Pro 1.0在图像推理上表现优异（纯图像97.0%）
  - **数据集一致性验证**（Kendall's tau）:
    - **Retrieval**: τ=0.87 (p=0.02*) - SMMQG与MMQA强一致
    - **QA**: τ=0.86 (p=0.002*) - SMMQG可替代MMQA用于模型选择
    - **结论**: SMMQG数据集可用于替代MMQA进行模型评估和选择
  - **关键发现**:
    - **评估数据质量**: SMMQG生成的合成数据质量与人工众包数据相当（在某些维度甚至更优）
    - **风格敏感性**: MMRAG性能高度依赖问题风格，需要风格特定的评估数据
    - **模态敏感性**: 检索和QA性能都显著依赖输入模态组合
    - **自动化评估**: 可实现大规模、定制化的MMRAG系统评估
  - **机构**: C3 AI、Connectly AI、卡内基梅隆大学
  - **作者**: Ian Wu, Sravan Jayanthi, Vijay Viswanathan, Simon Rosenberg, Sina Pakazad, Tongshuang Wu, Graham Neubig
  - **发布时间**: EMNLP 2024 Findings
  - **论文链接**: [ACL Anthology](https://aclanthology.org/2024.findings-emnlp.759/)
  - **意义**:
    - **评估数据生成**: 首个可控风格和模态的合成多模态QA数据生成框架，解决评估数据匮乏问题
    - **质量验证**: 通过人类研究证明合成数据质量可达众包水平
    - **性能洞察**: 揭示模型在特定风格和模态上的优势和劣势，无法通过混合基准获得
    - **可扩展性**: 完全自动化流程，可快速为新领域或新需求生成定制评估数据
    - **成本效益**: 无需昂贵的人工标注，降低评估数据集构建成本

---

- **📄 MMC** [(arXiv 2311.10774)](https://arxiv.org/abs/2311.10774) 🏷️ **[方法 + 数据]** - **腾讯AI实验室**
...
- **📄 AdaMLLM** [(arXiv 2411.19930)](https://arxiv.org/abs/2411.19930) 🏷️ **[方法 + 数据]** - **BIGAI**
...
- **📄 Multimodal ArXiv** [(arXiv 2403.00231)](https://arxiv.org/abs/2403.00231) 🏷️ **[方法 + 数据]** - **香港大学**
...
- **📄 PROVISION** [(arXiv 2412.07012)](https://arxiv.org/abs/2412.07012) 🏷️ **[方法 + 数据]**
...
- **📄 Self-Synthesized Data** [(arXiv 2502.14044)](https://arxiv.org/abs/2502.14044) 🏷️ **[方法 + 数据]** - **ICLR 2025**
...
- **📄 STIC (Self-Training on Image Comprehension)** [(arXiv 2405.19716)](https://arxiv.org/abs/2405.19716) 🏷️ **[方法 + 数据]** - **NeurIPS 2024**
...
- **📄 Genixer** [(arXiv 2312.06731)](https://arxiv.org/abs/2312.06731) 🏷️ **[方法 + 数据]**
...
- **📄 mmE5** [(arXiv 2502.08468)](https://arxiv.org/abs/2502.08468) 🏷️ **[方法 + 数据]**
...
- **📄 World to Code (W2C)** [(arXiv 2409.20424)](https://arxiv.org/abs/2409.20424) 🏷️ **[方法 + 数据]**
...
- **📄 SK-VQA** [(arXiv 2406.19593)](https://arxiv.org/abs/2406.19593) 🏷️ **[方法 + 数据]**
  - **聚焦**: **大规模合成知识生成，训练上下文增强多模态LLM** - 生成大规模合成上下文文档和知识型VQA对，用于训练多模态检索增强生成(RAG)系统
  - **数据合成方法** - **GPT-4引导的上下文文档与QA生成Pipeline**:
    - **核心创新**: 首个大规模方法，使用GPT-4自动为任意图像生成上下文文档和需要知识推理的QA对，实现超越Wikipedia链接图像限制的上下文增强多模态学习
    - **多源图像集成**:
      - **LAION图像**: 908K QA对，配GPT-4生成的上下文，覆盖多样化网络图像
      - **Wikipedia图像**: 702K合成上下文QA对 + 182K真实Wikipedia上下文QA对
      - **COCO-Counterfactuals**: 214K QA对，支持受控评估场景
      - **更广覆盖**: 支持任意图像源，相比现有数据集仅限Wikipedia可链接图像
    - **GPT-4上下文生成过程**:
      - **上下文文档创建**: 生成与图像相关的Wikipedia风格文章，不直接引用图像
      - **8条件QA生成**: 严格标准确保问题需要图像和上下文双重推理:
        1. 问题必须涉及图像
        2. 避免直接提及对象名称  
        3. 答案必须需要对上下文文档的推理
        4. 自然简洁的问题表述
        5. 从上下文文档中提取的答案
        6. 答案不能是图像中可见的对象
        7. 单词/短语答案，多个正确答案用逗号分隔
        8. 答案中不含连词('和'/'或')
    - **质量保证Pipeline**:
      - **多阶段过滤**: IR（图像检索）+ IR+CAP（图像检索+标题）过滤策略
      - **语法验证**: LanguageTool评估显示10K样本上下文文档93.31%语法准确率
      - **人工评估**: 完整数据集77%准确率，过滤子集87%（与其他VQA数据集相当）
      - **偏见与毒性检测**: 使用最先进模型筛查生成内容
      - **事实性验证**: 86%随机抽样QA对通过在线来源验证为事实正确
  - **技术实现**:
    - **上下文多样性**: 主题跨越40+类别，从体育到科学、艺术到地理
    - **问题独特性**: 96%的200万+问题独特（比Encyclopedic-VQA多11倍独特问题）
    - **词汇丰富度**: 926K独特POS序列，138K词汇量，12.7平均问题长度
    - **过滤有效性**: 过滤后保留984K最高质量QA对，同时保持多样性
  - **数据规模**:
    - **总数据集**: **200.6万QA对**（迄今最大的基于知识的VQA数据集）
    - **过滤子集**: SK-VQA IR (153万), SK-VQA IR+CAP (98.4万)
    - **上下文文档**: 每个QA对配有包含必要推理信息的丰富上下文文档
    - **图像来源**: 4个多样化来源确保领域覆盖和评估鲁棒性
  - **评估与影响**:
    - **挑战水平**: 比现有数据集更具挑战性 - 最先进MLLMs达到38-50%准确率 vs ViQuAE上47-84%
    - **泛化提升**: SK-VQA微调在InfoSeek、Enc-VQA、ViQuAE上一致提升跨模型规模的域外性能
    - **RAG系统训练**: 支持训练面向真实世界RAG场景的上下文增强多模态模型
    - **基准稳定性**: 相比现有数据集，在不同模型规模上提供更一致的评估
  - **关键技术贡献**:
    - **可扩展合成生成**: 证明百万级全自动上下文+QA生成的可行性
    - **多模态RAG训练**: 首个专为训练多模态RAG系统设计的数据集  
    - **质量-数量平衡**: 同时实现规模（200万+样本）和质量（高人工评估分数）
    - **领域泛化**: 多源方法确保跨不同图像类型和知识领域的鲁棒性能
  - **机构**: 多个机构合作
  - **开源**: ✅ [数据集与生成代码](https://arxiv.org/abs/2406.19593)
...
- **📄 AnyGPT** [(arXiv 2402.12226)](https://arxiv.org/abs/2402.12226) 🏷️ **[方法 + 数据]** - **复旦大学**
...
- **📄 TaskGalaxy** [(ICLR 2025)](https://arxiv.org/abs/2502.09925) 🏷️ **[方法 + 数据]** - **ICLR 2025 - 快手科技**
...
- **📄 Chart-CoCa** [(CIKM 2025)](https://arxiv.org/abs/2508.11975) 🏷️ **[方法 + 数据]** - **CIKM 2025**
  - **聚焦**: **通过代码驱动合成和候选条件应答实现图表理解自我改进** - VLM自我生成代码→执行生成图表→自动提取精确标注→候选条件应答训练，实现无需人工标注或外部模型的图表理解能力提升
  - **数据合成方法** - **代码中介的图表合成 + 候选条件应答训练**:
    - **核心创新**: 首个完全自我改进的图表理解范式，使用代码作为中介确保合成数据的精确性和可靠性，无需人工标注或教师模型
    - **问题识别**:
      - VLMs在图表理解任务上表现不佳（描述不准确、推理复杂度不足）
      - 传统合成数据方法存在噪声标签问题（直接图表→文本生成难以保证准确性）
      - 现有方法依赖强模型作为教师（如GPT-4）或大量人工标注
    - **代码驱动合成Pipeline（三步）**:
      1. **图表描述生成（Step 1: Chart Description）**:
         - **输入**: 未标注的真实图表c
         - **输出**: 图表描述d（包含标题、x/y轴标签、数据点、图例等）
         - **模型**: VLM M（初始版本，如InternVL2-8B）
         - **公式**: d = M_LMM(c)
         - **目的**: 将视觉信息转化为结构化文本描述
      2. **代码生成（Step 2: Code Generation）**:
         - **输入**: 图表描述d
         - **输出**: 可执行的Python Matplotlib代码p
         - **模型**: VLM的语言模型组件M_LM
         - **公式**: p = M_LM(d)
         - **约束**: 必须使用Python Matplotlib库（便于执行和信息提取）
         - **关键优势**: 代码可执行性确保生成的图表与描述精确匹配
      3. **代码执行与信息提取（Step 3: Execution & Info Extraction）**:
         - **图表生成**: 执行代码p生成新图表c*（与描述d精确匹配）
         - **信息提取**: 在代码执行过程中，通过反射(reflection)提取图表元素：
         ...
         - **QA对生成**:
           - 使用CharXiv的描述性问题作为种子（简单、无设计成本、易于访问答案）
           - 基于提取的信息自动生成精确答案
           - 示例：Q: "图表的标题是什么？" A: axes[0,0].get_title() → "Optimizations in SizeAware++"
           - 支持布局、标题、标签、图例、刻度、颜色条、线条等8类问题
         - **输出**: 高质量三元组⟨c*, q*, a*⟩（图表、问题、答案）
    - **关键技术优势**:
      - **零噪声标签**: 代码执行保证图表与标注的绝对一致性（vs 直接生成方法的噪声）
      - **完全自动化**: 无需人工标注或外部强模型
      - **可验证性**: 代码可追溯，每个标注都有明确来源
      - **可扩展性**: 可应用于任何未标注图表数据集
    - **候选条件应答（Candidate-Conditioned Answering）**:
      - **动机**: 直接微调存在问题 - 问题简单固定，模型难以学习处理灵活问题
      - **灵感**: Test-time scaling（增加推理预算提升性能）
      - **训练阶段（三步）**:
        1. **候选答案生成**: 给定⟨c*, q*⟩，初始VLM M生成k个候选响应r_1,...,r_k
           - 公式: r_1,...,k = M(c*, q*)
           - 使用采样策略（temperature>0）生成多样化候选
        2. **答案模型训练**: 训练新的答案VLM M_ANS，输入为(c*, q*, r_1,...,r_k)，输出最终答案a
           - 公式: a = M_ANS(c*, q*, r_1,...,r_k)
           - M_ANS学习综合多个候选并生成最优答案
           - 训练损失: L_answer = -log P(a* | c*, q*, r_1,...,r_k)
        3. **迭代更新**: 使用(c*, q*, a)更新初始VLM M的参数，获得M_ANS
      - **推理阶段（两步）**:
        1. 初始VLM M生成k个候选: r_1,...,k = M(c, q)
        2. 训练好的M_ANS综合候选生成最终答案: â = M_ANS(c, q, r_1,...,k)
      - **优势**: 
        - 增强模型处理灵活、多样化问题的能力
        - 利用多个候选的集体智慧
        - Test-time scaling带来性能提升
  - **实验结果** - **CharXiv数据集上大幅提升**:
    - **CharXiv基准**: 包含描述性任务（Information Extraction, Pattern Recognition, Enumeration, Counting, Compositionality）和推理任务（Text in Chart/General, Number in Chart/General）
    - **InternVL2-8B自我改进效果**:
      - **描述性任务**: 54.10% → **69.60%** (+15.50%)
        - Information Extraction: 69.40% → 74.20% (+4.80%)
        - Pattern Recognition: 40.52% → 75.46% (+34.94%，最大提升)
        - Enumeration: 19.64% → 26.79% (+7.15%)
        - Counting: 54.10% → 69.60% (+15.50%)
      - **推理任务**: 23.60% → **31.60%** (+8.00%)
      ...
      - **整体提升**: 平均+11.75%，证明完全自我改进范式的有效性
    - **跨模型泛化性验证**（使用Chart-CoCa方法）:
      - **LLaVA-1.6-7B**: 32.77% → **47.83%** (+15.06%, 描述性); 15.50% → **23.10%** (+7.60%, 推理)
      - **Qwen2VL-7B**: 59.90% → **71.75%** (+11.85%, 描述性); 30.80% → **36.90%** (+6.10%, 推理)
      - **InternVL2-26B**: 61.90% → **72.63%** (+10.73%, 描述性); 34.60% → **39.50%** (+4.90%, 推理)
      - **结论**: Chart-CoCa方法在不同规模和架构的模型上均有效
    - **与Majority Voting对比**:
      - Majority Voting（5候选）: 描述性61.38%, 推理26.70%
      - Chart-CoCa（5候选）: 描述性**69.60%**, 推理**31.60%**
      - **提升**: +8.22% (描述性), +4.90% (推理)
      - **原因**: Majority Voting容易陷入多数偏见；Chart-CoCa通过候选条件应答学习综合多样信息
    - **Pass@K分析（覆盖率）**:
      - **描述性任务**: Pass@1=54.1%, Pass@30=**85.95%** (+31.85%)
      - **推理任务**: Pass@1=23.6%, Pass@30=**55.8%** (+32.2%)
      - **结论**: VLM固有能力强（给足尝试次数可达高准确率），但需要方法挖掘这种潜力
    - **代码执行成功率分析**:
      - **总体成功率**: 70.7%（经过最多5次重试）
      - **错误类型分布**: ValueError (36.0%), IndexError (24.1%), SyntaxError (5.6%), AttributeError (5.0%), TypeError (2.9%), Others (1.5%)
      - **重试改进**: 单次尝试失败的样本中，24%通过重试成功；但部分样本陷入相同错误
      - **改进空间**: 针对性错误处理可进一步提升代码执行成功率
  - **消融实验关键发现**:
    - **代码中介的必要性**（-Code）:
      - 移除代码生成步骤，直接从图表生成QA: 描述性-9.20%, 推理-5.50%
      - **结论**: 代码中介确保数据精确性，显著优于直接生成
    - **图表描述的必要性**（-Desc）:
      - 移除图表描述步骤，直接从图表生成代码: 描述性-4.53%, 推理-3.60%
      - **结论**: 图表描述作为中间表示，降低直接生成代码的难度
    - **两者结合的重要性**（-Both）:
      - 同时移除描述和代码步骤: 描述性-13.68%, 推理-7.10%
      - **结论**: 完整Pipeline对性能至关重要
  - **定性分析**:
    - **候选答案多样性**: 5个候选展现不同推理路径，Chart-CoCa能综合选择最佳答案
    - **错误案例**: 主要集中在需要复杂推理或精确数值计算的问题（代码生成错误或逻辑错误）
    - **成功案例**: Chart-CoCa在多数描述性问题和中等难度推理问题上表现出色
  - **机构**: 香港科技大学（广州）
  - **作者**: Gongyao Jiang, Qiong Luo
  - **发布时间**: CIKM 2025
  - **代码**: [待开源]
  - **意义**:
    - **自我改进范式**: 首个完全自我改进的图表理解方法，无需人工标注或外部教师模型
    - **代码中介创新**: 证明使用代码作为中介可消除合成数据的噪声标签问题
    - **候选条件应答**: 创新的训练策略，利用test-time scaling思想提升模型泛化能力
    - **可扩展性**: 方法可应用于任何未标注图表数据，理论上可无限扩展训练数据
    - **跨模型泛化**: 在多种VLM架构上验证有效，提供通用的自我改进框架
    - **实用价值**: 为图表理解任务提供低成本、高质量的数据生成和模型改进方案

---

- **📄 VLM Dialog Games** [(arXiv 2502.02740)](https://arxiv.org/abs/2502.02740) 🏷️ **[方法 + 数据]**
...
- **📄 CSR (Calibrated Self-Rewarding)** [(arXiv 2405.14622)](https://arxiv.org/abs/2405.14622) 🏷️ **[方法 + 数据]** - **NeurIPS 2024**
